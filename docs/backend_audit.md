...
## 20) Рефакторинг: удаление легаси граф/telegram/stats кода и зачистка контейнера (ноябрь 2025)

Что сделано:

- Удалены неиспользуемые схемы Graph/Telegram из `service/presentation/routers/ml_api/schemas.py` — файл теперь содержит только ML-схемы.
- Легаси роутер статистики `service/presentation/routers/stats_api/*` выведен из эксплуатации (не подключался, зависел от отсутствующего сервиса).
- Зачищены placeholder'ы в контейнере: убраны `StatsServiceName`, `StatsRepositoryName`, `FileStorageServiceName`.
- Удалено легаси файловое хранилище `service/infrastructure/storage/file_storage.py` и связанный стек `service/scripts/core/*` (Envelope/Graph/Registry), не применялись в текущем приложении.

Проверка:

- Полный прогон тестов: PASS (19 passed, 1 skipped).
- Импорт и запуск приложения не затронуты (stats роутер не был подключён).

Оставшиеся моменты:

- Переходное имя `MLServiceName` оставлено как заглушка для обратной совместимости в lifecycle, планируем убрать после полной миграции на `TrainingService`.
- Продолжить зачистку репозиториев, которые ещё могут ссылаться на несуществующие модели/поля (см. первоначальные пункты аудита).
...
## 21) Репозитории: зачистка Stats* и финализация контейнера (ноябрь 2025)

Что сделано:

- Полностью вычищены placeholder'ы из контейнера: удалены `MLRepositoryName`, `StatsRepositoryName`, а также устаревший `MLServiceName`.
- Модули `stats_repository.py`, `stats_service.py`, `presentation/routers/stats_api/*` физически удалены.

Проверка:

- Полный прогон тестов: PASS (19 passed, 1 skipped) на момент изменений.
- Импорты в активном коде не затрагивают удалённые сущности.

Следующие шаги:

- Продолжить инвентаризацию репозиториев на предмет ссылок на несуществующие поля/модели (если такие проявятся в будущих изменениях — удалить/переписать).

---

## 22) Расширение метрик: confusion_matrix и финальная очистка легаси (ноябрь 2025)

Что сделано:

- Добавлено поле `confusion_matrix` (2D список int) в `MetricsResponse` для heavy-пути классификации.
- Heavy training: сохранение матрицы ошибок (`sklearn.metrics.confusion_matrix`) при классификации.
- Удалён устаревший `MLService` плейсхолдер и логика его shutdown в `app_lifespan`.
- Физически удалены файлы статистики (Stats*), вместо ImportError-заглушек.
- Исправлена индентация блока `except` в `health_check` после рефакторинга.

Проверка:

- Тесты: PASS (19 passed, 1 skipped) — добавление `confusion_matrix` не ломает существующие тесты (поле опционально, heavy тест остаётся пропущенным на Windows).
- Поиск ссылок на `MLServiceName` — отсутствуют.

Backlog / Возможные улучшения:

- Добавить тест на присутствие `confusion_matrix` (только в heavy окружении Linux с включённым `ENABLE_REAL_TRAINING`).
- Агрегировать сводную статистику по матрице (per-class precision/recall) — требует сохранения классов.
- Реализовать эндпоинт скачивания артефакта модели (с проверкой owner).

Риски:

- При большом числе классов матрица может быть крупной — потребуется лимит или усечение.
- Heavy путь всё ещё подвержен нативным крашам в специфичных окружениях (Windows + Python 3.13); решение — изолированный Linux CI.
