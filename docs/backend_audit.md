...
## 16) Актуализация: переход на лёгкий ML fallback и расширение тестового покрытия (ноябрь 2025)
...
- Эндпоинт загрузки датасета `/api/ml/v1/datasets/upload` переписан: валидация CSV теперь через стандартный модуль `csv` (исключены импорты pandas).
...
- Предупреждения: остаётся общий DeprecationWarning Pydantic о class-based config (источник во внешних зависимостях или моделях вне затронутых файлов; миграция на `ConfigDict` остаётся пунктом работ).
...
6. Добавить ретенцию артефактов (TTL / максимальное число моделей на пользователя).
7. Реализовать сбор агрегированных сравнительных метрик между запусками (история качества).
...
Зафиксировать лёгкий fallback до решения проблемы совместимости NumPy/Pandas (обновление бинарей или pin версии Python < 3.13). Параллельно готовить инфраструктуру для избирательного включения полноценного ML обучения.
