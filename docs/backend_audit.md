...
- Расширить метрики (precision/recall/F1) для классификации и `mae` для регрессии в heavy-пути.
- Добавить агрегирующие эндпоинты (сводные статистики по последним N запускам).
...
- Добавить «лучшие» показатели (например, `max_accuracy`, `max_r2`, `min_mse`) и разбиение по `task`.
